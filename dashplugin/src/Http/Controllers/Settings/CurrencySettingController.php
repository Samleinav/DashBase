...
class CurrencySettingController extends SettingController
{
    ...
    {
        $this->pageTitle(trans('plugins/dashplugin::currency.currencies'));

        $form = CurrencySettingForm::create();

        return view('plugins/dashplugin::settings.currency', compact('form'));
    }
    ...
    public function update(CurrencySettingRequest $request, BaseHttpResponse $response, StoreCurrenciesService $service)
    {
        ...
        if (! $currencies) {
            return $response
                ->setNextUrl(route('dashplugin.settings.currencies'))
                ->setError()
                ->setMessage(trans('plugins/dashplugin::currency.require_at_least_one_currency'));
        }
