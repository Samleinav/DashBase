...
use Botble\Dashplugin\Forms\Fronts\RoleFrontForm as RoleForm;
...
use Botble\Dashplugin\Models\Customer as User;
use Botble\Dashplugin\Tables\Front\RoleFrontTable as RoleTable;
...
class RolesCustomerController extends BaseController
{
    protected function breadcrumb(): Breadcrumb
    {
        return DashHelper::breadcrumb()
            ->add(
                trans('Settings'),
                route('public.settings.index')
            )
            ->add(
                trans('core/acl::permissions.role_permission'),
                route('public.settings.roles.index')
            );
    }
    ...
    public function index(RoleTable $dataTable)
    {
        ...
        return $dataTable->renderTable([], [], true);
    }
    ...
    public function edit(Role $role)
    {
        ...
        return RoleForm::createFromModel($role)->renderFront();
    }
    ...
    public function update(Role $role, RoleCreateRequest $request)
    {
        ...
        $role->updated_by = $request->user("customer")->getKey() ?? $request->user()->getKey();
        ...
        return $this
            ->httpResponse()
            ->setPreviousRoute('public.settings.roles.index')
            ->setNextRoute('public.settings.roles.edit', $role->getKey())
    ...
    public function create()
    {
        ...
        return RoleForm::create()->renderFront();
    }
    ...
    public function store(RoleCreateRequest $request)
    {
        ...
        $role = Role::query()->create([
            ...
            'created_by' => $request->user("customer")->getKey() ?? $request->user()->getKey() ,
            'updated_by' => $request->user("customer")->getKey() ?? $request->user()->getKey(),
        ]);
    ...
    public function getDuplicate(Role $role)
    {
        ...
        return $this->httpResponse()
            ->setPreviousRoute('public.settings.roles.edit', $role->getKey())
            ->setNextRoute('public.settings.roles.edit', $duplicatedRole->getKey())
